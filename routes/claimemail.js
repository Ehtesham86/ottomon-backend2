...
router.post("/", async (req, res, next) => {
  try {
      const claimemail = new Claimemail({
          ...
          from: req.body.message.headers.from,
          mailgunResponseId: req.body.id, // Use the ID from the Mailgun response
          sentAt: req.body.timestamp || new Date().toISOString(), // Use timestamp from Mailgun
          subject: req.body.message.headers.subject || "No Subject",
          type: req.body.type || "email",
          to: req.body.message.headers.to, // Correctly access the 'to' field
